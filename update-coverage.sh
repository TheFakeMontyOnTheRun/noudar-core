...
make clean
make CXX='g++' CXXFLAGS='-Inoudar-core/include -Incurses-version -g -c -std=c++0x -fprofile-arcs -ftest-coverage -I$(GTEST_DIR)/include -I$(GTEST_DIR) -I$(GTEST_DIR)/include/gtest -I$(GTEST_DIR)/include/gtest/internal -I$(GMOCK_DIR)/include -I$(GMOCK_DIR) -I$(GMOCK_DIR)/include/gtest -I$(GMOCK_DIR)/include/gtest/internal -IEASTL/include -IEASTL/test/packages/EABase/include/Common' LDFLAGS='-lncurses -lpthread --coverage' unittests
